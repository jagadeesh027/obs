...
import { Plus, Trash2, Mail, LayoutDashboard, Building2, X } from "lucide-react";
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const qProperties = query(collection(db, "properties"));
    ...
    const unsubscribeInquiries = onSnapshot(
      qInquiries, 
      (snapshot) => {
        setInquiries(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as any)));
      },
      (error) => {
  ...
  const handleDeleteProperty = async () => {
    ...
    try {
      ...
      handleFirestoreError(error, OperationType.DELETE, `properties/${deleteId}`);
    } finally {
      setIsDeleting(false);
    }
  };
              ...
              <div className="grid grid-cols-2 gap-8 py-8 border-y border-white/10">
                <div>
                  ...
                  <p className="text-sm font-light">
                    {selectedInquiry.createdAt?.toDate().toLocaleDateString('en-IN', {
                      day: 'numeric',
                      month: 'long',
                      year: 'numeric',
                      hour: '2-digit',
